...
<x-layouts.builder :title="$project->name">
    ...
    <header class="flex h-14 items-center gap-1 border-b border-zinc-800 bg-zinc-950 px-3">
        ...
        {{-- Project name + stack badge --}}
        <div class="mr-auto flex items-center gap-2.5">
            @if($lang)
                <div class="flex h-6 w-6 items-center justify-center rounded text-[10px] font-bold leading-none shrink-0"
                     style="background-color: {{ $lang['bg'] }}; color: {{ $lang['color'] }};">
                    {{ $lang['abbr'] }}
                </div>
            @endif
            <span class="text-sm font-medium text-zinc-200">{{ $project->name }}</span>
            @if($fwLabel)
                <span class="hidden rounded-full border border-zinc-700 bg-zinc-800 px-2 py-0.5 text-[10px] font-medium text-zinc-400 sm:inline">
                    {{ $fwLabel }}
                </span>
            @endif
        </div>
        ...
        <div class="flex overflow-hidden rounded-lg border border-zinc-700 bg-zinc-900">
            <button id="btn-desktop" data-active="true"
                    class="builder-viewport-btn flex items-center gap-1.5 bg-zinc-700 px-2.5 py-1.5 text-xs font-medium text-zinc-100 transition-colors"
                    title="Desktop">
        ...
        <button id="btn-preview"
                class="flex h-8 items-center gap-1.5 rounded-md border border-zinc-700 px-3 text-xs font-medium text-zinc-300 transition-colors hover:bg-zinc-800 hover:text-zinc-100">
            <svg class="h-3.5 w-3.5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.75" stroke="currentColor">
    ...
    <div class="flex" style="height: calc(100vh - 56px);">
        ...
        <aside class="flex w-60 shrink-0 flex-col border-r border-zinc-800 bg-zinc-900">
            <div class="flex border-b border-zinc-800">
                <button data-left-tab="blocks" aria-selected="true"
                ...
                <button data-left-tab="layers" aria-selected="false"
        ...
        <main class="relative flex-1 overflow-hidden">
            <div id="gjs"
                 ...
                 data-save-url="{{ route('projects.save') }}"
                 data-project-language="{{ $project->language }}"
                 data-project-framework="{{ $project->framework }}"
                 class="h-full w-full">
            </div>
        </main>

        {{-- ── Right: Styles + Traits + Code ──────────────────────── --}}
        <aside class="flex w-72 shrink-0 flex-col border-l border-zinc-800 bg-zinc-900">
            <div class="flex border-b border-zinc-800">
                <button data-right-tab="styles" aria-selected="true"
                        class="flex-1 border-b-2 border-zinc-400 py-2.5 text-xs font-semibold uppercase tracking-wider text-zinc-100 transition-colors">
                    Styles
                </button>
                <button data-right-tab="traits" aria-selected="false"
                        class="flex-1 py-2.5 text-xs font-semibold uppercase tracking-wider text-zinc-500 transition-colors hover:text-zinc-300">
                    Settings
                </button>
                <button data-right-tab="code" aria-selected="false"
                        class="flex-1 py-2.5 text-xs font-semibold uppercase tracking-wider text-zinc-500 transition-colors hover:text-zinc-300">
                    Code
                </button>
            </div>
            ...
            {{-- Code preview panel --}}
            <div data-right-panel="code" class="hidden flex-1 flex-col overflow-hidden">
                <div class="flex border-b border-zinc-800">
                    <button id="code-tab-html" data-code-tab="html"
                            class="flex-1 border-b-2 border-zinc-400 px-3 py-2 text-[10px] font-semibold uppercase tracking-wider text-zinc-100 transition-colors">
                        HTML
                    </button>
                    <button id="code-tab-css" data-code-tab="css"
                            class="flex-1 px-3 py-2 text-[10px] font-semibold uppercase tracking-wider text-zinc-500 transition-colors hover:text-zinc-300">
                        CSS
                    </button>
                </div>

                <div id="code-html-panel" class="flex-1 overflow-auto">
                    <pre id="code-html" class="p-3 text-[10px] leading-relaxed text-zinc-400 whitespace-pre-wrap break-all font-mono"></pre>
                </div>
                <div id="code-css-panel" class="hidden flex-1 overflow-auto">
                    <pre id="code-css" class="p-3 text-[10px] leading-relaxed text-zinc-400 whitespace-pre-wrap break-all font-mono"></pre>
                </div>

                <div class="border-t border-zinc-800 p-2">
                    <button id="btn-copy-code"
                            class="w-full rounded-lg border border-zinc-700 py-1.5 text-xs font-medium text-zinc-400 transition-colors hover:bg-zinc-800 hover:text-zinc-200">
                        Copy to clipboard
                    </button>
                </div>
            </div>
    ...
    {{-- Code panel tab wiring (separate from main panel tabs) --}}
    <script>
    document.addEventListener('DOMContentLoaded', function () {
        // Code sub-tabs
        document.querySelectorAll('[data-code-tab]').forEach(btn => {
            btn.addEventListener('click', () => {
                const target = btn.dataset.codeTab;
                document.querySelectorAll('[data-code-tab]').forEach(b => {
                    b.classList.toggle('border-b-2', b === btn);
                    b.classList.toggle('border-zinc-400', b === btn);
                    b.classList.toggle('text-zinc-100', b === btn);
                    b.classList.toggle('text-zinc-500', b !== btn);
                });
                document.getElementById('code-html-panel').classList.toggle('hidden', target !== 'html');
                document.getElementById('code-css-panel').classList.toggle('hidden', target !== 'css');
            });
        });

        // Copy button
        document.getElementById('btn-copy-code')?.addEventListener('click', () => {
            const htmlVisible = !document.getElementById('code-html-panel').classList.contains('hidden');
            const text = htmlVisible
                ? document.getElementById('code-html').textContent
